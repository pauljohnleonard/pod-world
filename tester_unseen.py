...
TIME_LIMIT=60.0*5     # Maximum time allowed for each attempt
TRIPS_LIMIT=300.0     # Goal in number of trip wires
...
BASE_MARK=40.0
PJL_SCORE=336
WORLD_DIR="unseen_world"
PUNTER_DIR="punters_unseen"
MUSIC_CMD="/Applications/VLC.app/Contents/MacOS/VLC"
MUSIC="MUSIC/Mission.mp3"

import operator,importlib,traceback,subprocess
...
if MAC_MUSIC:
        music_proc = subprocess.Popen([MUSIC_CMD, MUSIC,'-I','rc'])
...
def tester(world_name,punter_names,fout):
    ...
    for name in punter_names:
        ...
        try:
            punters_path=PUNTER_DIR+'/'+name
            os.chdir(punters_path)

            plug=importlib.import_module(PUNTER_DIR+'.'+name+'.plugin')
...
res_file="results_test/unseen_"+d_str+".txt"

res_file="Unseen_Results.txt"
...
for world_name in world_names:
    ...
    pod_list=tester(WORLD_DIR+"/"+world_name,punter_names,fout)         
