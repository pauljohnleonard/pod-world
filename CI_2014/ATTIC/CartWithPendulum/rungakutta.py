...
class RungeKutta:
    ...
    def stepRK(self):
        ...
        tstart = self.time
        self.model._eval(tstart, self.state, self.vel1)
        ...
        self.model._eval(tmid, self.state1, self.vel2)

        # 3. redo using new velocity estimate
        for i in xrange(self.n):
            self.state2[i] = self.state[i] + self.h * self.vel2[i] * 0.5   # k3
      
        self.model._eval(tmid, self.state1, self.vel3)

        # 4. estimate velocity at end
        tend = self.time + self.h;
        for i in xrange(self.n):
            
            #   CHECK NEXT LINE  ROBERTO Changed 0.5 to 1
            self.state3[i] = self.state[i] + self.h * self.vel3[i]
      
        self.model._eval(tend, self.state3, self.vel4)
